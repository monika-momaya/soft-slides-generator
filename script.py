...
app = base / 'app.py'
app.write_text('''import io, math, zipfile, re
from pathlib import Path
...
def ordered_indices_by_role(df):
    ...
    return sorted(df.index.tolist(), key=lambda i: (ROLE_PRIORITY.get(roles.loc[i].split()[0] if roles.loc[i] else '', 99), i))
...
def crop_to_cutout(photo, cutout_size):
    photo = ImageOps.fit(photo, cutout_size, method=Image.Resampling.LANCZOS, centering=(0.5, 0.33))
    return photo
...
def guess_row_fields(row):
    ...
    for v in vals:
        ...
        if any(k in up for k in ['MODERATOR', 'CHAIR', 'KEYNOTE', 'PANELIST', 'SPEAKER']):
            ...
            title_company = v if not title_company else title_company + ' | ' + v
    if not name and len(vals) >= 2:
        name = vals[0]
        title_company = vals[1] if len(vals) > 1 else ''
    title = title_company
    company = ''
    if '|' in title_company:
        title, company = [x.strip() for x in title_company.split('|', 1)]
...
if EXCEL is not None:
    ...
    if st.button('Generate Downloads'):
        ...
        cutout_bbox = cutout_img.getbbox()
        cutout_size = (cutout_img.size[0], cutout_img.size[1])
        W, H = base_img.size
        ...
        def render_slide(indices, parsed_rows):
            ...
            cols = 3 if len(indices) > 3 else max(1, len(indices))
            ...
            photo_w = int(min(cell_w, cell_h) * 0.48)
            photo_h = photo_w
